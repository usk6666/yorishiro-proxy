...
// The db must already have the plugin_kv table created (see EnsureTable).
...
// EnsureTable creates the plugin_kv table if it does not exist.
// This should be called once during initialization.
func EnsureTable(ctx context.Context, db *sql.DB) error {
	if _, err := db.ExecContext(ctx, createPluginKVSQL); err != nil {
		return fmt.Errorf("create plugin_kv table: %w", err)
	}
	return nil
}
...
func (ps *PluginStore) loadCache(ctx context.Context) error {
	...
	rows, err := ps.db.QueryContext(ctx,
		"SELECT key, value FROM plugin_kv WHERE plugin_name = ?", ps.pluginName)
	if err != nil {
...
func (ps *PluginStore) storeClear(_ *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	_, err := ps.db.ExecContext(context.Background(),
		"DELETE FROM plugin_kv WHERE plugin_name = ?", ps.pluginName)
	if err != nil {
