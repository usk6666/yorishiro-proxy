...
func TestQuery_Flows_InvalidScheme(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)

	result := callQuery(t, cs, queryInput{
		Resource: "flows",
		Filter:   &queryFilter{Scheme: "ftp"},
	})
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid scheme")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid scheme") {
		t.Errorf("error should mention invalid scheme, got: %s", text.Text)
	}
}

func TestQuery_Flows_InvalidState(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)

	result := callQuery(t, cs, queryInput{
		Resource: "flows",
		Filter:   &queryFilter{State: "pending"},
	})
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid state")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid state") {
		t.Errorf("error should mention invalid state, got: %s", text.Text)
	}
}

func TestQuery_Flows_InvalidBlockedBy(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)

	result := callQuery(t, cs, queryInput{
		Resource: "flows",
		Filter:   &queryFilter{BlockedBy: "firewall"},
	})
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid blocked_by")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid blocked_by") {
		t.Errorf("error should mention invalid blocked_by, got: %s", text.Text)
	}
}

func TestQuery_Flows_InvalidSortBy(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)
	...
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid sort_by")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid sort_by") {
		t.Errorf("error should mention invalid sort_by, got: %s", text.Text)
	}
	if !strings.Contains(text.Text, "timestamp") {
		t.Errorf("error should list valid sort_by values, got: %s", text.Text)
	}
}
...
func TestQuery_FuzzResults_InvalidSortBy(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)
	...
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid fuzz_results sort_by")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid sort_by") {
		t.Errorf("error should mention invalid sort_by, got: %s", text.Text)
	}
	if !strings.Contains(text.Text, "index_num") {
		t.Errorf("error should list valid sort_by values, got: %s", text.Text)
	}
}

func TestQuery_FuzzJobs_InvalidStatus(t *testing.T) {
	store := newTestStore(t)
	cs := setupQueryTestSession(t, store)

	result := callQuery(t, cs, queryInput{
		Resource: "fuzz_jobs",
		Filter:   &queryFilter{Status: "done"},
	})
	if !result.IsError {
		t.Fatal("expected IsError=true for invalid fuzz_jobs status")
	}
	text := result.Content[0].(*gomcp.TextContent)
	if !strings.Contains(text.Text, "invalid status") {
		t.Errorf("error should mention invalid status, got: %s", text.Text)
	}
	if !strings.Contains(text.Text, "running") {
		t.Errorf("error should list valid status values, got: %s", text.Text)
	}
}
