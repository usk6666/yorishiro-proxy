...
var availableResources = []string{"sessions", "session", "messages", "status", "config", "ca_cert", "intercept_queue"}
...
func (s *Server) registerQuery() {
	gomcp.AddTool(s.server, &gomcp.Tool{
		...
		Description: "Unified information query tool. Retrieve sessions, session details, messages, " +
			"proxy status, configuration, CA certificate, or intercept queue. " +
			"Set 'resource' to one of: sessions, session, messages, status, config, ca_cert, intercept_queue. " +
