...
type queryFilter struct {
	// Protocol filters flows by application protocol (e.g. "HTTP/1.x", "HTTP/2", "gRPC", "WebSocket", "TCP").
	Protocol string `json:"protocol,omitempty" jsonschema:"application protocol filter (e.g. HTTP/1.x, HTTP/2, gRPC, WebSocket, TCP, SOCKS5+HTTPS, SOCKS5+HTTP)"`
	// Scheme filters flows by URL scheme / transport security (e.g. "https", "http", "wss", "ws", "tcp").
	// Use scheme to find all TLS flows regardless of protocol (e.g. scheme=https returns HTTP/1.x, HTTP/2, gRPC over TLS).
	Scheme string `json:"scheme,omitempty" jsonschema:"URL scheme / transport filter (e.g. https, http, wss, ws, tcp)"`
...
func (s *Server) registerQuery() {
	gomcp.AddTool(s.server, &gomcp.Tool{
		...
		Description: "Unified information query tool. Retrieve flows, flow details, messages, " +
			...
			"The 'filter' parameter supports filtering flows by protocol (HTTP/1.x, HTTP/2, gRPC, WebSocket, TCP), scheme (https, http, wss, ws, tcp — use scheme to find all TLS flows regardless of protocol), method, url_pattern, status_code, blocked_by (target_scope, intercept_drop, rate_limit), state (active, complete, error), technology (e.g. nginx, wordpress), conn_id (connection ID, exact match), and host (matches server_addr or URL host); " +
