...
// --- isHTTPLikeProtocol tests ---

func TestIsHTTPLikeProtocol(t *testing.T) {
	tests := []struct {
		protocol string
		want     bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.protocol, func(t *testing.T) {
			got := isHTTPLikeProtocol(tt.protocol)
			if got != tt.want {
				t.Errorf("isHTTPLikeProtocol(%q) = %v, want %v", tt.protocol, got, tt.want)
			}
		})
	}
}

// --- expandRawParamsWithKVStore tests ---

func TestExpandRawParamsWithKVStore_HTTPProtocol(t *testing.T) {
	// "GET / HTTP/1.1\r\nHost: §host§\r\n\r\n" in base64
	rawText := "GET / HTTP/1.1\r\nHost: §host§\r\n\r\n"
	rawBase64 := base64.StdEncoding.EncodeToString([]byte(rawText))

	params := &resendParams{
		OverrideRawBase64: rawBase64,
	}
	kvStore := map[string]string{
		"host": "example.com",
	}

	err := expandRawParamsWithKVStore(params, kvStore, "HTTP")
	if err != nil {
		t.Fatalf("expandRawParamsWithKVStore() error = %v", err)
	}
	...
	if string(decoded) != want {
		t.Errorf("expanded = %q, want %q", string(decoded), want)
	}
}

func TestExpandRawParamsWithKVStore_NonHTTPProtocol(t *testing.T) {
	rawText := "§host§"
	rawBase64 := base64.StdEncoding.EncodeToString([]byte(rawText))
	...
	err := expandRawParamsWithKVStore(params, kvStore, "TCP")
	if err != nil {
		t.Fatalf("expandRawParamsWithKVStore() error = %v", err)
	}

	// Should not expand for non-HTTP protocol.
	if params.OverrideRawBase64 != rawBase64 {
		t.Errorf("override_raw_base64 was modified for TCP protocol")
	}
}

func TestExpandRawParamsWithKVStore_EmptyKVStore(t *testing.T) {
	rawBase64 := base64.StdEncoding.EncodeToString([]byte("test"))
	params := &resendParams{
		OverrideRawBase64: rawBase64,
	}

	err := expandRawParamsWithKVStore(params, nil, "HTTP")
	if err != nil {
		t.Fatalf("expandRawParamsWithKVStore() error = %v", err)
	}

	// Should not modify when kvStore is empty.
	if params.OverrideRawBase64 != rawBase64 {
		t.Errorf("override_raw_base64 was modified with empty kvStore")
	}
}

func TestExpandRawParamsWithKVStore_EmptyOverrideRawBase64(t *testing.T) {
	params := &resendParams{
		OverrideRawBase64: "",
	}
	...
	if params.OverrideRawBase64 != "" {
		t.Errorf("override_raw_base64 should remain empty")
	}
}

func TestExpandRawParamsWithKVStore_HTTP2Protocol(t *testing.T) {
	rawText := "data=§token§"
	rawBase64 := base64.StdEncoding.EncodeToString([]byte(rawText))
	...
	err := expandRawParamsWithKVStore(params, kvStore, "HTTP/2")
	if err != nil {
		t.Fatalf("expandRawParamsWithKVStore() error = %v", err)
	}
...
func TestExpandRawParamsWithKVStore_SOCKS5PlusHTTPS(t *testing.T) {
	rawText := "Host: §host§"
	rawBase64 := base64.StdEncoding.EncodeToString([]byte(rawText))
	...
	err := expandRawParamsWithKVStore(params, kvStore, "SOCKS5+HTTPS")
	if err != nil {
		t.Fatalf("expandRawParamsWithKVStore() error = %v", err)
	}
