...
func TestBuildPoolHitFastPath_TakesFastPathWhenH2Enabled(t *testing.T) {
	...
	cached, teardown := startCachedH2Layer(t, envelope.EnvelopeContext{
		ConnID:     "cached-conn",
		TargetHost: target,
		TLS:        &envelope.TLSSnapshot{ALPN: "h2"},
	})
	defer teardown()
	hostTLSForKey, err := resolvePerHostTLS(target, buildCfg)
	if err != nil {
	...
	// Use a closed pipe so performClientMITM fails quickly (the test only
	// needs to observe the entry into the function, not a successful
	// handshake).
	_, clientConn := net.Pipe()
	_ = clientConn.Close()
	...
	if ferr == nil {
		t.Errorf("expected non-nil error from performClientMITM with closed clientConn")
	}

	if got := buildCfg.ClientMITMHandshakeCount(); got != 1 {
		t.Errorf("ClientMITMHandshakeCount = %d, want 1 (fast path should have entered MITM)", got)
	}
}
