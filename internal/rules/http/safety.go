...
func (e *SafetyEngine) CheckInput(msg *envelope.HTTPMessage) *Violation {
	e.mu.RLock()
	defer e.mu.RUnlock()

	for _, rule := range e.rules {
		if v := e.checkRule(&rule, msg); v != nil {
			return v
		}
	}
	return nil
...
func (e *SafetyEngine) CheckInputAll(msg *envelope.HTTPMessage) []*Violation {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (e *SafetyEngine) checkRule(rule *common.CompiledRule, msg *envelope.HTTPMessage) *Violation {
...
func (e *SafetyEngine) extractTarget(target common.Target, msg *envelope.HTTPMessage) (data, targetName string) {
	switch target {
	case common.TargetBody:
		if msg.Body == nil {
			return "", ""
		}
		return string(msg.Body), "body"
