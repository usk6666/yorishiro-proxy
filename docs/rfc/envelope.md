...
**Acceptance record — `HostScopeStep` scheme derivation (USK-1081, 2026-09-15).** "Protocol-agnostic" constrains *what the Step may read*, not *how little it may pass on*. Until USK-1081, `HostScopeStep` handed `TargetScope.CheckTarget` a blank scheme, on the reasoning that a connection-level check "only validates hostname and port". `matchTargetRule` is AND logic, so every rule carrying a `Schemes` condition silently stopped matching there: allow rules blocked everything (`CheckTarget` step 3 — "policy allow rules exist and none match"), and deny rules blocked nothing. The example allow rule published in `internal/mcp/resources/help_security.md` is `Schemes`-bearing, so an operator who copied it got a 403 on every request.

The scheme is now derived from `Context.TLS` — non-nil ⇒ `https`, nil ⇒ `http`. `TargetRule.Schemes` means **transport confidentiality**, plaintext versus TLS, spelled with the `http` / `https` tokens; it is not the L7 application protocol. That is already the meaning the connection-level handler gates encode with their literals (`connectPolicyAllow` scopes a CONNECT tunnel as `"https"` even when the tunnel carries SMTP; the plain-HTTP forward handler scopes as `"http"`). Deriving it from `Context.TLS` answers the same question without inspecting `Message`, so the §3.5.1 contract holds. Because TLS is per-Layer and not per-stack (§3.1), the two legs of a connection with independent TLS axes derive different schemes — that is each leg's wire reality, and it is what a `Schemes` rule is asking about.

Observable effect on existing operator configs: deny rules become strictly stricter, allow rules strictly more permissive, in both cases converging on exactly what the operator authored. No configuration that works today stops working, because a `Schemes`-bearing rule previously matched nothing at this Step in either direction.

Two adjacent sites were considered and deliberately **not** changed. (1) `matchTargetRule`'s blank-scheme semantics: for an unknown scheme, deny wants "matches everything" and allow wants "matches nothing", so any single rule is fail-open in exactly one direction; an asymmetric per-list semantic could be fail-safe both ways but is a behaviour change to a live policy engine with deliberate blank callers. **Re-open trigger:** a caller appears that genuinely cannot know the scheme *and* needs `Schemes` rules to apply to it. (2) SOCKS5's own inline check (`internal/connector/socks5.go`) still passes a blank scheme, because `REP=0x02` must precede any tunnel byte and at that moment the tunneled protocol is genuinely unknown — an accepted and already-documented limitation.
