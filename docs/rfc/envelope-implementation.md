...
2. On graceful EOF inside `session.wsRelayDirection`, `session.clientToUpstream`, or `session.upstreamToClient`, looks up the OPPOSITE direction on the same Stream within a 2-second correlation window AND verifies that the relay loop observing the EOF has NOT yet successfully forwarded an envelope on its own direction (USK-860 gate). A hit means "the upstream half-closed shortly after we forwarded a long-held frame AND no opposite-direction envelope was relayed between release and EOF" — i.e. the post-release wire conversation never made round-trip progress. When the gate trips (an envelope WAS relayed between release and EOF), the tracker entry is still cleared but the callback is suppressed so a healthy modify_and_forward → 200 OK → Connection: close exchange is not mislabelled.
...
**解決 (approach B, USK-855):** per-protocol hold-timeout overrides under the existing `intercept_queue` config block. `HoldQueue` resolves `(timeout, behavior)` per-call from the held `Envelope.Protocol`, falling back per-field to the global. Built-in defaults seed `ws=60s`, `sse=60s`, `grpc=60s` as a human-review window for AI-agent intercept loops; USK-854's hold-window keepalive injection now keeps the upstream alive during the hold, so the original 8s edge-idle safety margin (USK-855) is no longer required (USK-863). File-config block: `intercept_queue.protocol_overrides`; MCP tool: `configure intercept_queue.protocol_overrides` (merge: per-key delta with null-clear; replace: full-overwrite). Friction-list update site for canonical key set: `internal/config/intercept_queue.go` (godoc). Smoke-tier e2e: `internal/layer/ws/intercept_hold_timeout_protocol_override_integration_test.go`.

**Out of scope (deferred follow-ups):**
- Approach A (WS Ping/Pong keepalive injection during holds) — **Implemented in USK-854** as the opt-in preventive overlay. Disabled by default (`web_socket.hold_keepalive_enabled=false`); plugin opt-out via `ctx.stream_state["ws_hold_keepalive"]=False` from a `(ws, on_upgrade, pre)` hook. USK-851 (approach D, observability) remains RESOLVED — the two features are orthogonal.
  - **Plugin visibility scope (known limitation):** Synthetic Pings carry `Context.Synthetic=true` (per RFC-001 provenance) but are written directly via `wsSendSerializer.Send` and bypass the Pipeline. PluginPre/Post therefore do NOT observe synthetic Pings on the Send side; operators see the wire-observed Pong reflection on the Receive side (where it flows through the full Pipeline including PluginPre/Post and RecordStep). The `Context.Synthetic` guards on `InterceptStep` / `SafetyStep` exist as defensive boundary code (in case any future code path routes a synthetic envelope into the Pipeline) but are unreachable in the current wiring. A follow-up Issue can extend Send-side Pipeline integration if operator scenarios demand symmetric visibility — the design tradeoff there is balancing per-tick Pipeline allocation cost against operator observability.
...
**状態:** **RESOLVED** (USK-851 + USK-855, 2026-05-13). Approach D implementation in `internal/rules/common/release_tracker.go`, `internal/session/session.go` (`checkInterceptReleaseEOF`), and `internal/proxybuild/builder.go::buildSessionOptions`. Approach B implementation in `internal/rules/common/queue.go` (per-protocol resolution), `internal/config/intercept_queue.go` (file-config), `internal/mcp/configure_tool.go` (`protocol_overrides`), and `internal/mcpserver/intercept_queue_init.go` (file-config → HoldQueue plumbing). Smoke-tier e2e tests: `internal/layer/ws/intercept_hold_eof_integration_test.go` (D), `internal/layer/ws/intercept_hold_timeout_protocol_override_integration_test.go` (B). USK-860 (2026-05-13) narrowed Approach D's firing condition with a per-relay-direction "did we relay an envelope between release and EOF?" gate so successful end-to-end exchanges (e.g. HTTP modify_and_forward → 200 OK → upstream Connection: close) are no longer mislabelled. Regression guard: `internal/layer/http1/intercept_hold_outcome_integration_test.go`. USK-863 (2026-05-13) raised the WS/SSE defaults in `internal/rules/common/queue.go` from 8s to 60s now that USK-854's keepalive injection eliminates the edge-idle race that motivated the original 8s safety margin; the per-protocol-override structure (USK-855) is preserved unchanged. Regression guard: `internal/layer/ws/intercept_hold_keepalive_integration_test.go` (smoke tier, USK-854).
