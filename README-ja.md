...
すべてのプロキシ操作は 10 個の MCP ツールとして公開されます:
...
| `resend` | 記録されたリクエストをミューテーション付きでリプレイ（メソッド/URL/ヘッダ/ボディのオーバーライド、JSON パッチ、生バイトパッチ、ドライラン） |
...
| `security` | プロキシの到達範囲を制限する Target Scope ルール（Policy Layer + Agent Layer）の設定 |
