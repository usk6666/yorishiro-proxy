...
2. **全プロトコルで raw bytes の記録・閲覧・改変が可能であること** — 診断ツールとして、プロトコルレベルの異常検出・再現ができなければならない
3. **L7 パースは raw bytes の上に乗るオーバーレイであり、raw bytes を破壊してはならない** — 記録された raw bytes は常にワイヤー上の元データを反映する
...
| SOCKS5 | N/A | N/A (トランスポート層) | HTTP 委譲後は適用 |
